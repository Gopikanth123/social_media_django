...
def index(request):
    ...
    #     post.profile_pic = user_profile.profile_pic
    #     posts_with_profile_pic.append(post)
    ...
    if request.method=='POST':
        ...
        if password1==password2:
            if User.objects.filter(username=username).exists():
                ...
                #create a pofile object for new user...
...
def like_post(request):
    ...
    if like_filter==None:
        ...
        new_like.save()
        post.no_of_posts=post.no_of_posts+1
        post.save()
        return redirect('/')
    else:
        like_filter.delete()
...
def profile(request, pk):
    ...
    return render(request, 'profile.html',matter)
